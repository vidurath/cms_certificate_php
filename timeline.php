...
<!-- timeline start -->
  <div class="timeline">
    ...
    <!-- speedlimiter information [Add to system for the first time] -->
    <?php
      $sql1="SELECT day(createdAt) as day,month(createdAt) as month,year(createdAt) as year,TIME(createdAt) as sptime FROM speedlimiter WHERE serialnumber='$_GET[sn]'";
    ...
    $result=$conn->query($sqlmain);
     ...
     <div class="time-label">
     <span class="bg-green"><?php echo $rowsn['day']; ?> <?php echo month_name($spdate); ?> <?php echo $rowsn['year']; ?></span>
     </div>
     ...
     <!-- comments -->
         ...
         $result2=$conn->query($sql2);
           if($result2->num_rows > 0) {
             while($rownote = $result2->fetch_assoc()) {
               $spdate = $rownote['month'];
          ?>
          ...
          <span class="time"><i class="fas fa-calendar"></i> <?php echo $rownote['day'] ?>- <?php echo month_name($spdate); ?> -<?php echo $rownote['year'] ?></span>
         ...
         <div class="timeline-body">
           ...
           $result3=$conn->query($sql3);
             if($result3->num_rows > 0) {
               while($rowimage = $result3->fetch_assoc()) {
            ?>
